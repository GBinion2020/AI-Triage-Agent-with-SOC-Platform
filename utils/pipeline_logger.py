...
class PipelineLogger:
    ...
    def __init__(self, alert_id: str, log_dir: str = "pipeline_logs"):
        ...
        self.agent_io_file = os.path.join(log_dir, f"agent_io_{alert_id}_{timestamp}.jsonl")
...
"""
    ...
    def log_agent_io_exact(self, agent_name: str, agent_input: Dict[str, Any], agent_output: Dict[str, Any]):
        """
        Persist exact agent input/output payloads (full-fidelity) to JSONL.
        """
        payload = {
        ...
        with self.lock:
            try:
                with open(self.agent_io_file, "a", encoding="utf-8") as f:
                    f.write(json.dumps(payload, default=str) + "\n")
            except Exception as e:
                print(f"[LOGGER ERROR] Failed to write agent I/O audit for {agent_name}: {e}")

        self.log_step(
            "AGENT_IO_AUDIT",
            f"Persisted exact I/O for {agent_name} to {self.agent_io_file}",
        )
