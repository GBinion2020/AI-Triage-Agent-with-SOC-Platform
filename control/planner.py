...
class DeterministicPlanner:
    ...
    def _ioc_allowed_for_vt(self, ioc_type: str, value: str) -> bool:
        if not ioc_type or not value:
            return False
        if ioc_type == "ip":
            return self._is_public_ip(value)
        return ioc_type in {"domain", "hash"}

    def _append_vt_if_allowed(self, plan: List[Dict[str, Any]], state: InvestigationState, indicator: str, ioc_type: str) -> bool:
        if not self._ioc_allowed_for_vt(ioc_type, indicator):
            return False
        args = {"indicator": indicator, "type": ioc_type}
        if state.is_duplicate("check_virustotal", args):
            return False
        plan.append({"tool": "check_virustotal", "args": args})
        return True
    ...
    def plan_from_intent(self, intent: str, state: InvestigationState) -> List[Dict[str, Any]]:
        ...
        if not iocs["ip"] and not iocs["domain"] and not iocs["hash"]:
            for item in state.ioc_store:
                ...
                if not self._ioc_allowed_for_vt(ioc_type, value):
                    continue
        ...
        for ioc_type in ["hash", "domain", "ip"]:
            for value in iocs.get(ioc_type, []):
                if self._append_vt_if_allowed(plan, state, value, ioc_type):
                    return plan
        ...
        if target_dimension == "process" and state.ioc_store:
            ioc = next(
                (
                    i
                    for i in state.ioc_store
                    if self._ioc_allowed_for_vt(i.get("type"), i.get("value"))
                ),
                None,
            )
            if ioc:
                self._append_vt_if_allowed(plan, state, ioc["value"], ioc["type"])
            ...
            for ioc_type in ["hash", "domain", "ip"]:
                for value in iocs.get(ioc_type, []):
                    if self._append_vt_if_allowed(plan, state, value, ioc_type):
                        return plan
            for item in state.ioc_store:
                if self._append_vt_if_allowed(plan, state, item.get("value"), item.get("type")):
                    return plan
