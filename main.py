...
# Components
from intake.ingest import AlertFetchError, AlertIngestor
...
def main() -> int:
    ...
    try:
        ...
        print(f"[pipeline] fetched_alerts={len(alerts)}")
    except AlertFetchError as e:
        logger.error(f"Failed to fetch alerts: {e}")
        print("[pipeline] result=fetch_error")
        return 2
    except Exception as e:
        logger.exception(f"Unexpected error while fetching alerts: {e}")
        print("[pipeline] result=fetch_error")
        return 2

    if not alerts:
        logger.info("No new alerts to process.")
        print("[pipeline] result=no_alerts processed_alerts=0")
        return 0
    ...
    soc2_decision_agent = SOC2DecisionAgent(llm_client)
    
    mitre_rag = MitreRAG()
    policy_engine = PolicyEngine()
    orchestration_service = OrchestrationService(llm_client, policy_engine)
    ...
    for alert in alerts:
        try:
            process_alert(
                ...
                inv_agent, reason_agent, decision_agent, soc2_decision_agent, orchestration_service,
                email_notifier
            )
            processed_alerts += 1
        except Exception as e:
            logger.exception(f"Unhandled error while processing alert {getattr(alert.alert, 'id', 'unknown')}: {e}")
            print("[pipeline] result=processing_error")
            return 3

    print(f"[pipeline] result=ok processed_alerts={processed_alerts}")
    return 0
...
def extract_iocs(state: InvestigationState) -> list:
    ...
    def is_probable_domain(value: str) -> bool:
        ...
        if "\\" in host or "/" in host or " " in host:
            return False
        if not re.fullmatch(r"(?=.{1,253}$)(?:[a-z0-9](?:[a-z0-9-]{0,61}[a-z0-9])?\.)+[a-z]{2,24}", host):
            return False
        tld = host.rsplit(".", 1)[-1]
        if tld in blocked_file_like_tlds:
            return False
        return True
    ...
    if state.alert.analysis_signals.external_destination:
        ...
        if dest not in seen_values:
            ioc_type = "domain" if is_probable_domain(dest) and not is_public_ip(dest) else "ip"
...
def run_legacy_investigation_loop(
    alert,
    state,
    techniques,
    policy_engine,
    ...
    """
    Legacy fixed-step investigation loop retained for rollback safety.
    """
    ...
    while state.iteration_count < max_loops:
        ...
        if op_conf_label == "medium":
            ...
            pipeline_log.log_agent_io_exact(
                "ReasoningAgent",
                {
                    ...
                    "prompt": getattr(reason_agent, "last_prompt", ""),
                },
                {
                    "raw_response": getattr(reason_agent, "last_raw_response", ""),
                    "parsed_output": final_reasoning,
                    "error": getattr(reason_agent, "last_error", ""),
                },
            )
        ...
        pipeline_log.log_agent_io_exact(
            "InvestigationAgent",
            {
                ...
                "iteration": state.iteration_count,
                "evidence_count": len(state.evidence),
                "tool_history_tail": [f"{t.tool_name}:{t.status}" for t in state.tool_history[-5:]],
                "prompt": getattr(inv_agent, "last_prompt", ""),
            },
            {
                "raw_response": getattr(inv_agent, "last_raw_response", ""),
                "parsed_output": intent,
                "error": getattr(inv_agent, "last_error", ""),
            },
        )
        ...
        if not plan_items:
            ...
            if len(state.audit_trail) >= 2:
                ...
                if all("No tools planned for this intent." in err for loop in last_two for err in loop.errors):
...
def process_alert(
    ...
    pipeline_log.log_agent_io_exact(
        "IntakeAgent",
        {
            ...
            "iteration": state.iteration_count,
            "evidence_count": len(state.evidence),
            "prompt": getattr(intake_agent, "last_prompt", ""),
        },
        {
            "raw_response": getattr(intake_agent, "last_raw_response", ""),
            "parsed_decision": intake_decision,
            "error": getattr(intake_agent, "last_error", ""),
        },
    )
    ...
    final_reasoning = ""
    orchestration_report = None
    pipeline_arch = os.getenv("PIPELINE_ARCH", "orchestrated").strip().lower()

    if pipeline_arch == "legacy":
        final_reasoning = run_legacy_investigation_loop(
            alert=alert,
            state=state,
        ...
        pipeline_log.log_section("SOC ORCHESTRATED INVESTIGATION")
        orchestration_report = orchestration_service.run(state=state, pipeline_log=pipeline_log)
        final_reasoning = orchestration_report.evidence_summary
        state.iteration_count = len(orchestration_report.waves)
        pipeline_log.log_data(
            "ORCHESTRATION_ARTIFACTS",
            [ref.path for ref in orchestration_report.artifact_refs][:20],
        )
             
    # --- Step 13: Final Decision ---
    pipeline_log.log_section("FINAL DECISION")
    logger.info("Generatng Final Decision...")
    decision_agent_name = "SOC2DecisionAgent" if orchestration_report else "DecisionAgent"
    agent_start = pipeline_log.log_agent_start(decision_agent_name, "Generating final verdict and recommendations")
    start_time = time.time()
    state.scoring_factors = build_risk_factors(state)
    state.risk_score = RiskScoringMatrix.calculate_score(state.scoring_factors)
    state.evidence_table = build_evidence_table(state.scoring_factors)
    ...
    if orchestration_report:
        final_output = soc2_decision_agent.decide(
            state=state,
            orchestration_summary=final_reasoning,
            artifact_refs=[ref.model_dump() for ref in orchestration_report.artifact_refs],
            scoring={
    ...
    pipeline_log.log_agent_end(decision_agent_name, agent_start, f"Classification: {final_output.get('classification', 'unknown')}")
    pipeline_log.log_agent_io_exact(
        decision_agent_name,
        {
            "alert_id": state.alert.alert.id,
            ...
            "risk_score": state.risk_score,
            "classification": final_classification,
            "prompt": getattr(soc2_decision_agent if orchestration_report else decision_agent, "last_prompt", ""),
        },
        {
            "raw_response": getattr(soc2_decision_agent if orchestration_report else decision_agent, "last_raw_response", ""),
            "parsed_output": final_output,
            "error": getattr(soc2_decision_agent if orchestration_report else decision_agent, "last_error", ""),
        },
    )
    ...
    pipeline_log.log_step(
        "FINAL_OUTPUT",
        (
            f"Classification: {final_output.get('classification', 'unknown')}\n"
            f"   Final Score: {final_output.get('final_score', 'unknown')}\n"
            f"   Action: {final_output.get('action', 'unknown')}\n"
            f"   Confidence Score: {final_output.get('confidence_score', 'unknown')}"
        ),
    )
